"""
Anthropic -> OpenAI 异步代理（asyncio + aiohttp 单进程版）——o2a 包内引擎模块。

原 proxy_async.py 整体移入 o2a 包；协议/配置/统计逻辑位于 o2a 包的
base.py / config.py / convert.py / stats.py。根目录 proxy_async.py 保留为兼容入口。
...
from .base import STREAM_TIMEOUT, _responses_url, logger
...
from .stats import get_account_summary, get_stats, is_cache_stats_enabled
